...
std::shared_ptr<SubTexture2D> SubTexture2D::create_from_coords(
    const std::shared_ptr<Texture2D>& texture, const glm::vec2& coords,
    const glm::vec2& cellSize, const glm::vec2& spriteSize)
{
    glm::vec2 min = {coords.x * cellSize.x / texture->get_width(),
                     coords.y * cellSize.y / texture->get_height()};
    glm::vec2 max = {
        (coords.x + spriteSize.x) * cellSize.x / texture->get_width(),
        (coords.y + spriteSize.y) * cellSize.y / texture->get_height()};
