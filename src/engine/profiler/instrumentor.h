...
struct SAMUI_PROFILER_API ProfileResult
{
    std::string Name;
...
class SAMUI_PROFILER_API Instrumentor
{
  public:
                      ...
                      const std::string& filepath = "results.json")
    {
        ...
        if (m_CurrentSession)
        {
            ...
            if (Log::GetEngineLogger())  // Edge case: BeginSession() might be
                                         // before Log::Init()
            {
                SAMUI_ENGINE_ERROR(
...
class SAMUI_PROFILER_API InstrumentationTimer
{
  public:
...
template <size_t N>
struct SAMUI_PROFILER_API ChangeResult
{
    char Data[N];
};

template <size_t N, size_t K>
constexpr auto SAMUI_PROFILER_API CleanupOutputString(const char (&expr)[N],
                                                  const char (&remove)[K])
{
