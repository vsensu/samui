...
OpenGLTexture2D::OpenGLTexture2D(uint32_t width, uint32_t height,
    ...
    : width_(width),
      height_(height),
      internal_format_(TextureFormatToOpenGLBaseType(format)),
      format_(format)
{
    ...
    uint16_t cell_width, uint16_t cell_height, uint16_t rows, uint16_t columns)
{
    ...
    //   // }
    // }
    // glTexImage2D(GL_TEXTURE_2D, 0 /*mipmap*/, texture->internal_format_,
    // width,
...
ImageInfo* OpenGLTexture2D::load_file(const std::filesystem::path& path)
{
    SAMUI_PROFILE_FUNCTION();
...
OpenGLCubeMap::OpenGLCubeMap(const std::array<std::filesystem::path, 6>& paths)
{
    ...
    stbi_uc* data = nullptr;
    {
        ...
        for (int i = 0; i < paths.size(); ++i)
        {
            ...
            SAMUI_ENGINE_ASSERT(data, "Failed to load image:{0}",
                                paths[i].string());
            GLenum internal_format =
                channels == 4 ? GL_RGBA : (channels == 3 ? GL_RGB : 0);
            SAMUI_ENGINE_ASSERT(internal_format,
                                "Format not supported! channels: {0}",
                                channels);
            glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0 /*mipmap*/,
                         internal_format, width, height, 0 /*legacy*/,
