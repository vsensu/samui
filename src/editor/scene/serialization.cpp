...
void SerializeEntity(YAML::Emitter& out, Scene& scene, Entity entity)
{
    ...
    if (scene.has_component<NameComponent>(entity))
    {
        out << YAML::Key << "NameComponent";
        out << YAML::BeginMap;  // NameComponent Map

        auto& name = scene.get_component<NameComponent>(entity).name;
    ...
    if (scene.has_component<TransformComponent>(entity))
    {
        ...
        auto& tc = scene.get_component<TransformComponent>(entity);
    ...
    if (scene.has_component<CameraComponent>(entity))
    {
        ...
        auto& cameraComponent = scene.get_component<CameraComponent>(entity);
    ...
    if (scene.has_component<SpriteRendererComponent>(entity))
    {
        ...
        auto& spriteRendererComponent =
            scene.get_component<SpriteRendererComponent>(entity);
...
bool deserialize_scene(Scene& scene, const std::string& filepath)
{
    ...
    if (entities)
    {
        for (auto entity : entities)
        {
            ...
            SAMUI_ENGINE_TRACE("Deserializing entity with ID = {0}, name = {1}",
                               uuid, name);

            auto deserialized_entity = scene.create_entity(name);

            auto transformComponent = entity["TransformComponent"];
            if (transformComponent)
            {
                // Entities always have transforms
                auto& tc = scene.get_component<TransformComponent>(
                    deserialized_entity);
                tc.translation =
            ...
            {
                auto& cc =
                    scene.add_component<CameraComponent>(deserialized_entity);

                cc.projection_type = (CameraComponent::ProjectionType)
            ...
            {
                auto& src = scene.add_component<SpriteRendererComponent>(
                    deserialized_entity);
